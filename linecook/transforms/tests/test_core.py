...
class TestFilterLine:
    ...
    def test_call_on_match(self):
        ...
        transform = core.filter_line(
            r"match", on_match=on_match, on_mismatch=on_mismatch
        )
    ...
    def test_call_on_mismatch(self):
        ...
        on_mismatch = mock.Mock()
        transform = core.filter_line(
            r"mismatch", on_match=on_match, on_mismatch=on_mismatch
        )
...
class TestPartition:
    ...
    def test_partition_output(self):
        match_word = self.create_partition_function(
            "([A-z]+)", success_template="yes({})", failure_template="no({})"
        )
        assert match_word("1 match 2") == "no(1 )yes(match)no( 2)"

    def create_partition_function(
        self, match_pattern, success_template="{}", failure_template="{}"
    ):
        ...
        return core.partition(
            match_pattern, on_match=self.on_match, on_mismatch=self.on_mismatch
        )
...
class TestColorText:
    def test_color_text(self):
        ...
        assert (
            color_red("match words in this line")
